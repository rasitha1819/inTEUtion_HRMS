...
const Dashboard = () => {
  const { user, isHRorAdmin, isEmployee, todayAttendance, punchCheckIn, punchCheckOut } = useAuth();
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Banner */}
      <div className="relative overflow-hidden rounded-3xl border border-indigo-500/20 bg-gradient-to-r from-indigo-900/40 via-purple-900/20 to-slate-900/40 p-6 sm:p-8 backdrop-blur-xl">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-semibold text-indigo-400 uppercase tracking-wider">Dashboard Overview</span>
          ...
          <div className="flex items-center gap-3">
            {!todayAttendance.checkedIn ? (
              <button
                ...
                className="flex items-center gap-2 rounded-2xl bg-emerald-500 px-5 py-3 text-sm font-bold text-slate-950 shadow-lg shadow-emerald-500/25 hover:bg-emerald-400 transition-all transform hover:scale-105 disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={punchLoading}
                className="flex items-center gap-2 rounded-2xl bg-rose-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-rose-500/25 hover:bg-rose-400 transition-all transform hover:scale-105 disabled:opacity-50"
              >
                <Square className="h-4 w-4 fill-current" />
                <span>{punchLoading ? 'Recording...' : 'Punch Out Now'}</span>
              </button>
            ) : (
              <div className="flex items-center gap-2 rounded-2xl bg-indigo-500/20 border border-indigo-500/30 px-5 py-3 text-sm font-semibold text-indigo-300">
        ...
        {punchMsg && (
          <p className="mt-3 text-xs font-medium text-indigo-300 bg-indigo-950/60 inline-block px-3 py-1 rounded-lg border border-indigo-500/20">
            {punchMsg}
          </p>
        )}
      </div>
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-900/60 p-6 backdrop-blur-xl">
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-400">Daily present vs late attendance logs</p>
            </div>
            ...
            {trends.map((t, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-1 flex-col items-center gap-2 group">
                  {/* Tooltip on hover */}
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -translate-y-12 bg-slate-950 text-slate-200 text-[10px] px-2 py-1 rounded shadow-lg border border-slate-800 whitespace-nowrap z-20 pointer-events-none">
                    {t.date}: {t.present} Present, {t.late} Late
                  </div>

                  {/* Stacked bar */}
                  <div className="w-full max-w-[36px] flex flex-col items-center justify-end rounded-t-lg overflow-hidden bg-slate-800/40 h-44">
                    {t.late > 0 && (
                      <div
                  ...
                  <span className="text-[11px] font-medium text-slate-400">{t.day}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 backdrop-blur-xl">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              {isHRorAdmin && (
                <Link to="/departments" className="text-xs text-indigo-400 hover:text-indigo-300 font-semibold flex items-center gap-1">
            ...
            <div className="mt-4 space-y-3">
              {depts.slice(0, 4).map((d) => (
                <div key={d.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-950/50 border border-slate-800/60">
                  <div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 backdrop-blur-xl">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link
                to="/attendance"
                className="flex items-center gap-2 rounded-xl bg-slate-800/50 p-2.5 text-xs font-medium text-slate-300 hover:bg-slate-800 hover:text-white border border-slate-700/40 transition-colors"
              >
