...
const LeaveApprovalModal = ({ isOpen, onClose, leaveRequest, onSuccess }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Review Leave Application" maxWidth="max-w-lg">
      ...
      <div className="rounded-xl bg-slate-50 dark:bg-slate-950/70 p-4 border border-slate-200 dark:border-slate-800 space-y-2 mb-4">
        <div className="flex justify-between items-center">
          <span className="text-xs font-semibold text-slate-900 dark:text-white">{leaveRequest.employee_name} ({leaveRequest.employee_code})</span>
          <span className="text-xs text-indigo-600 dark:text-indigo-400 font-semibold">{leaveRequest.leave_type_display}</span>
        </div>
        <p className="text-xs text-slate-700 dark:text-slate-300">
          Duration: <span className="font-semibold text-slate-900 dark:text-white">{formatDate(leaveRequest.start_date)}</span> to{' '}
          <span className="font-semibold text-slate-900 dark:text-white">{formatDate(leaveRequest.end_date)}</span> ({leaveRequest.total_days} days)
        </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Decision selector */}
        <div>
          <label className="block text-xs font-semibold uppercase text-slate-700 dark:text-slate-400 mb-2">Review Decision *</label>
            ...
            <button
              ...
              className={`rounded-xl p-3 border text-xs font-semibold flex items-center justify-center gap-2 transition-all ${
                decision === 'APPROVE'
                  ...
                  : 'border-slate-300 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-400 hover:border-slate-400 dark:hover:border-slate-700'
              }`}
            >
              Approve Request
            </button>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`rounded-xl px-5 py-2 text-xs font-semibold text-white shadow-md transition-all disabled:opacity-50 ${
              decision === 'APPROVE'
