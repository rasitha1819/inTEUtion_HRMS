        ...
        {balances.length > 0 ? (
          balances.map((b) => {
            ...
            return (
              <div
                key={b.id}
                className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-4 backdrop-blur-xl transition-all hover:border-slate-300 dark:hover:border-slate-700 shadow-sm"
              >
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 truncate block max-w-[110px]" title={b.leave_type_display}>
                      {b.leave_type_display}
                    </span>
                    <h3 className="mt-1 text-xl font-extrabold text-slate-900 dark:text-white">
                      {b.remaining_days}{' '}
                      <span className="text-xs font-normal text-slate-500 dark:text-slate-400">/ {b.total_days}d</span>
                    </h3>
                  </div>
                  <span className="text-[11px] font-bold text-indigo-700 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 px-1.5 py-0.5 rounded border border-indigo-200 dark:border-indigo-500/20">
                    {b.used_days} used
                  </span>
                </div>
                ...
                <div className="mt-3">
                  ...
                  <div className="mt-1 flex justify-between text-[10px] text-slate-500 dark:text-slate-400">
                    <span>Avail: {b.remaining_days}d</span>
          ...
          <div className="col-span-full rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-4 text-center text-xs text-slate-500 dark:text-slate-400">
          ...
          <>
            ...
            >
              ...
              {pendingRequests.length > 0 && (
                <span className="rounded-full bg-amber-500/20 text-amber-700 dark:text-amber-300 px-2 py-0.2 text-[10px] border border-amber-500/30">
            ...
            if (list.length === 0) {
              return (
                <div className="p-12 text-center">
                  <CalendarDays className="mx-auto h-12 w-12 text-slate-300 dark:text-slate-600 mb-3" />
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">No applications in this view</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Submit a new leave request whenever needed.</p>
                </div>
              );
            }
                    ...
                    {list.map((req) => (
                      <tr key={req.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="py-4 px-4 font-semibold text-slate-900 dark:text-white">
                          ...
                          <span className="text-[10px] font-mono text-indigo-600 dark:text-indigo-400">{req.employee_code}</span>
                        </td>
                        ...
                        <td className="py-4 px-4 font-mono font-bold text-indigo-600 dark:text-indigo-300">
                        ...
                        <td className="py-4 px-4 text-right">
                          {isHRorAdmin && req.status === 'PENDING' ? (
                            <button
                              ...
                              className="rounded-lg bg-indigo-50 dark:bg-indigo-600/20 px-3 py-1 text-xs font-semibold text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30 hover:bg-indigo-600 hover:text-white transition-colors"
                            >
                              Review
                            </button>
                            ...
                            <button
                              ...
                              className="rounded-lg bg-rose-50 dark:bg-rose-500/10 px-3 py-1 text-xs font-semibold text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30 hover:bg-rose-600 hover:text-white transition-colors"
                            >
                              Cancel
                            </button>
                          ) : (
