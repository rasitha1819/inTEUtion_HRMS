    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-12 text-center">
          ...
          <p className="text-sm font-semibold text-slate-300">No departments configured</p>
          <p className="text-xs text-slate-500 mt-1">Create your first department using the button above.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {departments.map((dept) => (
            <div
              key={dept.id}
              className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 backdrop-blur-xl transition-all hover:border-slate-700 flex flex-col justify-between"
            >
              <div>
                ...
                <h3 className="mt-4 text-lg font-bold text-white">{dept.name}</h3>
              ...
              <div className="mt-6 pt-4 border-t border-slate-800/80 space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-white">{dept.employee_count || 0}</span>
                </div>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">Department Name *</label>
            <input
              type="text"
              required
              value={name}
          ...
          <div>
            <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">Code / Prefix *</label>
            <input
              type="text"
              required
              value={code}
              ...
              placeholder="e.g. ENG"
              className="w-full rounded-xl border border-slate-800 bg-slate-950 py-2 px-3 text-xs text-white uppercase focus:border-indigo-500 focus:outline-none"
            />
          </div>

          <div>
            <label className="block text-xs font-semibold uppercase text-slate-400 mb-1">Department Head / Manager</label>
            <select
              value={manager}
              onChange={(e) => setManager(e.target.value)}
              className="w-full rounded-xl border border-slate-800 bg-slate-950 py-2 px-3 text-xs text-slate-300 focus:border-indigo-500 focus:outline-none"
            >
